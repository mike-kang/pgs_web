...
class TranferHandle {
  ...
  mousemove(self, e) {
    ...
    if (self.state == TranferHandle.STATE_ROTATION) {
      ...
      self.angle += a1 - a0;
      //console.log(TranferHandle.TAG, "mousemove", self.angle);
      self.onChangeAngle(a1 - a0);
    }
    //클릭한 곳이 중앙점이라면
    else if (self.state == TranferHandle.STATE_MOVE) {
      self.onChangeMove(point.x - self.prePoint.x, point.y - self.prePoint.y);
    }
    self.prePoint = point;
  }
  ...
  draw(self, clear=true){
    console.log(TranferHandle.TAG, "draw", self);
    ...
    //move handle
    ctx.beginPath();
    ctx.arc(c_x, c_y, self.move_handle_radius, 0, 2 * Math.PI);
    ctx.fillStyle = "green";
    ctx.fill();
    ctx.closePath();
    ...
    //rotation circle
    ctx.beginPath();
    ctx.arc(c_x, c_y, self.rotation_r, 0, 2 * Math.PI);
    ...
    ctx.beginPath();
    ctx.arc(c_x + self.rotation_r * Math.cos(self.angle) , c_y + self.rotation_r * Math.sin(self.angle), self.rotation_handle_radius, 0, 2 * Math.PI);
    ctx.fillStyle = "white";
    ctx.fill();
